...
export function fetchIssueList() {
  return dispatch => {
    ...
    axios({
      ...
      .catch(err =>
        dispatch({ type: FETCH_LIST_FAILURE, errorMessage: err.response.data })
      );
  };
}
...
export function fetchListByEndpoint(endpoint) {
  return dispatch => {
    dispatch({ type: FETCH_LIST_REQUEST });
    const url = "http://localhost:8888/api/v1" + endpoint;
    axios({
      method: "get",
      url: url,
      headers: {
