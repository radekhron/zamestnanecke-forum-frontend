...
class ThemeEdit extends Component {
  ...
  componentDidMount() {
    const { fetchIssueList, fetchObjectDetailAndUpdateFormModel, resetModelDetail } = this.props;
    resetModelDetail('formData.theme');
    fetchIssueList();
    if (this.props.match.params.themeID) {
      fetchObjectDetailAndUpdateFormModel('/admin/theme/',this.props.match.params.themeID,'formData.theme');
    }
  }
        ...
        <div className="col-md-6 col-md-offset-3">
          <div className="well">
            <Form
              ...
              onSubmit={v => postObjectForCreateOrUpdate('/admin/theme/',v, history, '/admin/temata/seznam')}
              className="form-horizontal"
            >
              <div className="form-group">
                <label htmlFor=".name" className="form-label">Nadpis tématu:</label>
                <Control.text
                  model=".name"
                  required
                  id="theme.name"
              ...
              <div className="form-group">
                <label htmlFor=".description" className="form-label">Detail tématu:</label>
                <Control.textarea
                  model=".description"
                  required
              ...
              <div className="form-group">
                <label htmlFor=".linkedIssue" className="form-label">Navázaná šablona:</label>
                <Control.select
                  model=".linkedIssue"
                  required
                  className="form-control"
              ...
              <div className="form-group">
                ...
                <Control.select model=".state" required className="form-control">
                  ...
                  <option value="Next voting season">Hlasování v další sezoně</option>
                  <option value="Voting in progress" disabled>Probíhá hlasování</option>
                  <option value="Finished" disabled>Dokončeno</option>
                </Control.select>
              </div>
              ...
              {!isPosting &&
                <div className="form-group">
                  <button type="submit" className="btn btn-default btn-lg btn-block">
                    {this.props.match.path === '/admin/temata/vytvorit' && <span>Vytvořit </span>}
                    {this.props.match.path != '/admin/temata/vytvorit' && <span>Upravit </span>}
                     téma
                  </button>
                </div>
              }
